...
def llm_plan_next_action(
    ...
    with timing_stage("planner.total"):
        available_action_list = sorted(set(available_actions))
        with timing_stage("planner.runtime_state", actions=len(available_action_list)):
            planner_state = build_planner_runtime_state(state, available_action_list)
        with timing_stage("planner.action_mask", actions=len(available_action_list)):
            masked_actions, mask_reasons = mask_actions(planner_state, available_action_list)
        with timing_stage("planner.context_build", actions=len(masked_actions)):
        ...
        with timing_stage("planner.prompt_format", actions=len(masked_actions)):
        ...
        with timing_stage("planner.llm_invoke", actions=len(masked_actions)):
            planner_response = llm.invoke(planner_prompt.to_messages())
        with timing_stage("planner.parse_response", actions=len(available_action_list)):
        ...
        if (
            masked_ranked_actions
            and final_action == masked_ranked_actions[0]
            and final_action != planner_action
        ):
